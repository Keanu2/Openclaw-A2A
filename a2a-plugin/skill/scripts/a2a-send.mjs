...
const MAX_INLINE_FILE_SIZE = 50 * 1024 * 1024; // 50MB (align with plugin security.maxInlineFileSizeBytes)
...
async function main() {
  ...
  if (filePath) {
    ...
    if (stat.size > MAX_INLINE_FILE_SIZE) {
      console.error(`File too large: ${(stat.size / 1048576).toFixed(1)}MB exceeds 50MB limit`);
      process.exit(2);
    }
