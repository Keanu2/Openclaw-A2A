...
import { createTunnelFetch, type TunnelSession } from "./tunnel/index.js";
...
export class A2AClient {
  ...
  constructor(config?: { poolConfig?: ConnectionPoolConfig; tunnel?: TunnelSession | null }) {
    this.poolConfig = config?.poolConfig;
    this.tunnel = config?.tunnel ?? null;
  }

  /** Attach / replace tunnel session (e.g. after service start). */
  setTunnel(tunnel: TunnelSession | null): void {
    this.tunnel = tunnel;
  }
  ...
  private createFetch(peer: PeerConfig): typeof fetch {
    let baseFetch = this.createBaseFetch();
    if (peer.tunnelDeviceId && this.tunnel) {
      baseFetch = createTunnelFetch(this.tunnel, peer.tunnelDeviceId);
    }
  ...
  private buildFactory(peer: PeerConfig): { factory: ClientFactory; path: string } {
    ...
    const useTunnel = Boolean(peer.tunnelDeviceId && this.tunnel);
    ...
    const options = ClientFactoryOptions.createFrom(ClientFactoryOptions.default, {
      ...
      transports: useTunnel
        ? [
            // Tunnel only carries HTTP; skip gRPC for relay peers
  ...
  private async doSendMessage(
    ...
    try {
      ...
      // Tunnel peers: HTTP only (relay protocol does not carry gRPC)
      if (peer.tunnelDeviceId && this.tunnel) {
        transports = transports.filter((ep) => {
          const t = String(ep.transport).toUpperCase();
          return t !== "GRPC" && !t.includes("GRPC");
        });
      }
    } catch {
