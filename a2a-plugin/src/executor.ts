...
/** Common wrong / alternate MIME labels (models often send image/jpg). */
...
/** Sniff common file magic so images don't fall through to ".bin". */
function sniffExtensionFromBytes(buffer: Buffer): string | null {
 ...
 * filename extension, content sniffing, or ".bin".
 */
function extensionForMime(
  ...
  const mapped = MIME_TO_EXT[canonical];
  if (mapped) return mapped;

  // Fallback: use extension from original filename if it looks reasonable
  const ext = path.extname(originalName).toLowerCase();
  if (ext && ext.length <= 10 && /^\.[a-z0-9]+$/i.test(ext)) return ext;

  if (buffer) {
    const sniffed = sniffExtensionFromBytes(buffer);
...
function sanitizeFileName(rawName: string): string {
  const base = path.basename(String(rawName || "file").replace(/\\/g, "/"));
  const cleaned = base
    .replace(/[\r\n\t\x00-\x1f]/g, "")
    .replace(/[<>:"|?*]/g, "_")
    .replace(/^\.+/, "")
...
/**
 * Build a unique path under dir using the original file name.
 * Collision: "a.jpg" → "a (1).jpg" → "a (2).jpg".
 */
function uniquePathForName(dir: string, desiredName: string, mimeType: string, buffer: Buffer): string {
  let name = sanitizeFileName(desiredName);
  const mimeExt = extensionForMime(mimeType, name, buffer);
...
/**
 * Decode a base64-encoded inline file and save it under fileTempDir,
 * keeping the original file name when possible.
...
function saveBase64File(
  ...
  try {
    ...
    const filePath = uniquePathForName(fileTempDir, name, mimeType, buffer);
    fs.writeFileSync(filePath, buffer, { mode: 0o664 });
    // Match directory owner so File Manager (non-root) can open the file.
    try {
      const dirStat = fs.statSync(fileTempDir);
      fs.chownSync(filePath, dirStat.uid, dirStat.gid);
    } catch {
      // chown may fail on some hosts; ignore
    }
    return filePath;
  } catch {
    return null;
...
function formatFilePartAsText(obj: Record<string, unknown>): string {
  ...
  // Some transports drop `name` after large `bytes`; also accept common aliases.
  const rawName =
    asString(file.name) ||
  ...
  if (uri) {
    ...
    return (
      ...
      `URI: ${uri}\n` +
      `\n` +
      `请在聊天窗口用中文告知用户：已收到文件引用（URI），并给出 URI。`
    );
  }

  // Base64-encoded inline file: decode and save under fileTempDir (original name)
  const bytes = asString(file.bytes);
  if (bytes) {
    const savedPath = saveBase64File(bytes, name, mimeType);
    if (savedPath) {
      const sizeBytes = decodedBase64Size(bytes);
      const sizeKB = Math.ceil(sizeBytes / 1024);
      const savedBase = path.basename(savedPath);
      return (
        `【A2A 文件接收成功】\n` +
        `传输文件名: ${name}\n` +
        `落盘文件名: ${savedBase}\n` +
        `类型: ${mimeType}\n` +
        `大小: ${sizeBytes} bytes (${sizeKB}KB)\n` +
        `保存路径: ${savedPath}\n` +
        `\n` +
        `请在聊天窗口用中文明确告知用户：接收成功。\n` +
    ...
    return (
      `【A2A 文件接收失败】无法落盘保存 ${name} (${mimeType}, ~${sizeKB}KB)。` +
      `请在聊天窗口告知用户接收失败。`
    );
  }
