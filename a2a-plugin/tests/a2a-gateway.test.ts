...
describe("a2a-gateway plugin", () => {
  ...
  it("inbound FilePart (base64) is formatted with size hint", async () => {
    ...
    try {
      ...
      assert.ok(
        capturedMessage.includes(".pdf"),
        "saved file should have correct extension",
      );
    } finally {
      (globalThis as any).WebSocket = originalWebSocket;
    }
  ...
  it("a2a.send_local_file rejects relative and disallowed paths", async () => {
    const harness = createHarness(
      makeConfig({
        peers: [{ name: "peer-1", agentCardUrl: "http://mock-peer/.well-known/agent-card.json" }],
      })
    );

    const relative = await invokeGatewayMethod(harness, "a2a.send_local_file", {
      peer: "peer-1",
  ...
  it("a2a.send_local_file reads local path and sends FilePart bytes", async () => {
    ...
    // Must live under an allowlisted root (/data/local/...). On Windows write to C:\data\local\...
    const dir =
      process.platform === "win32"
        ? path.join("C:\\", "data", "local", "tmp", "a2a-send-local-file-test")
        : "/data/local/tmp/a2a-send-local-file-test";
    fs.mkdirSync(dir, { recursive: true });
    const filePath = path.join(dir, "hello.txt");
    fs.writeFileSync(filePath, "hello-a2a-local", "utf8");
    const sendPath =
      process.platform === "win32"
        ? filePath
        : "/data/local/tmp/a2a-send-local-file-test/hello.txt";

    const received: Array<Record<string, unknown>> = [];
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === "string" ? input : input.toString();
      if (url.includes("agent-card") || url.includes("agent.json")) {
        return new Response(
          JSON.stringify({
            protocolVersion: "0.3.0",
            name: "Peer Agent",
      ...
      if (url.includes("/a2a/jsonrpc")) {
        ...
        received.push(payload);
        return new Response(
          JSON.stringify({ jsonrpc: "2.0", id: payload.id, result: { accepted: true } }),
          { status: 200, headers: { "content-type": "application/json" } }
        );
      }
      return new Response("not found", { status: 404 });
    ...
    try {
      ...
      const result = await invokeGatewayMethod(harness, "a2a.send_local_file", {
        peer: "peer-1",
        path: sendPath,
        text: "local-file-test",
      });
      ...
      const parts = params?.message?.parts as Array<Record<string, unknown>>;
      assert.ok(Array.isArray(parts));
      const filePart = parts.find((p) => p.kind === "file") as any;
      assert.ok(filePart?.file?.bytes);
      assert.equal(
        Buffer.from(String(filePart.file.bytes), "base64").toString("utf8"),
        "hello-a2a-local"
      );
    } finally {
      globalThis.fetch = originalFetch;
